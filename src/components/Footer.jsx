...
const FooterLink = ({ label }) => {
  ...
  useEffect(() => {
    ...
    gsap.set(second, { y: 25, opacity: 0 });
    const tl = gsap.timeline({ paused: true });
    tl.to(
      first,
      { y: -25, opacity: 0, duration: 0.3, ease: "power2.inOut" },
      0,
    ).to(second, { y: 0, opacity: 1, duration: 0.3, ease: "power2.inOut" }, 0);
  ...
  return (
    <li
      ref={linkRef}
      className="relative overflow-hidden cursor-pointer h-7 flex items-center"
    >
      <span
        ref={firstTextRef}
        className="block font-bold text-[1.1rem] text-white whitespace-nowrap"
      >
        {label}
      </span>
      <span
        ref={secondTextRef}
        className="absolute block font-bold text-[1.1rem] text-[#b2f6e3] whitespace-nowrap"
      >
        {label}
      </span>
    </li>
...
// নিচের ছোট লিঙ্কগুলোর জন্য নতুন কম্পোনেন্ট (অ্যানিমেশন সহ)
const BottomLink = ({ children }) => (
  <p className="relative cursor-pointer group overflow-hidden">
    {children}
    <span className="absolute bottom-0 left-0 w-full h-[1px] bg-white transform translate-x-[-101%] group-hover:translate-x-0 transition-transform duration-300 ease-out"></span>
  </p>
);

function Footer() {
  const socialIcons = ["f", "X", "in", "y", "t", "i"];

  const links = {
    col1: ["Services", "Work", "About", "Culture", "Meet The Risers"],
    col2: ["Testimonials", "Blog", "Webinars", "Careers"],
    col3: ["Sheffield", "Manchester", "London", "New York", "Contact"],
  };
      ...
      <div className="bg-[#111212] rounded-3xl overflow-hidden">
        ...
        <div className="mx-4 md:rounded-[60px] border border-gray-800/30 p-6">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start gap-12 mb-10">
            ...
            <div className="w-full lg:w-[30%]">
              ...
              <div className="relative flex items-center mb-6 group/input">
                <input
                  ...
                  className="w-full bg-[#1e1f1f] rounded-full py-4 px-6 border-none outline-none text-white italic placeholder:text-gray-500"
                />
                <button className="absolute right-2 bg-[#b2f6e3] text-black w-10 h-10 flex items-center justify-center rounded-full overflow-hidden">
                  {/* Arrow Animation */}
                  <svg
                    width="20"
                    height="20"
                    ...
                    stroke="currentColor"
                    strokeWidth="3"
                    className="transition-transform duration-300 group-hover/input:translate-x-1 group-hover/input:-translate-y-1"
                  >
              ...
              <div className="flex flex-wrap gap-1.5">
                {socialIcons.map((icon, i) => (
                  <div
                    key={i}
                    className="bg-white text-black px-2.5 py-1 rounded-full text-[10px] font-black flex items-center gap-1 cursor-pointer hover:bg-[#b2f6e3] transition-colors"
                  >
            ...
            {/* Right: Links */}
            <div className="w-full lg:w-[60%] grid grid-cols-1 sm:grid-cols-3 gap-3 md:border-l md:border-gray-500 md:pl-3">
              <ul className="space-y-2 md:border-r md:border-gray-500">
                {links.col1.map((item) => (
        ...
        <div className="px-10 py-8 flex flex-col md:flex-row justify-between items-center text-[10px] text-accent gap-4 tracking-[1px]">
          <div className="flex flex-wrap justify-center md:justify-start gap-x-4 gap-y-2 items-center">
            <BottomLink>© 2025 Rise at Seven Ltd. All rights reserved</BottomLink>
            <span className="hidden md:inline">•</span>
            <div>Company Number 11955187</div>
            <span className="hidden md:inline">•</span>
            <div>VAT Registered GB 322402945</div>
            <span className="hidden md:inline">•</span>
            <BottomLink>Privacy Policy</BottomLink>
            <span className="hidden md:inline">•</span>
            <BottomLink>Terms & conditions</BottomLink>
          </div>
          <BottomLink>Website MadeByShape</BottomLink>
        </div>
      </div>
    </footer>
  );
